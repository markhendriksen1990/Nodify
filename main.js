...
function formatPositionData(data, walletAddress) {
    ...
    if (data.positionHistoryAnalysisSucceeded) {
        ...
        message += ` Initial Investment: $${data.histPrincipalUSD.toFixed(2)}\n`;
    } else {
    ...
    message += `\nPrice Information\n`;
    message += ` Range: $${lowerPrice.toFixed(4)} - $${upperPrice.toFixed(4)} ${data.t1.symbol}/${data.t0.symbol}\n`;
    message += ` Current Price: $${currentPrice.toFixed(4)} ${data.t1.symbol}/${data.t0.symbol}\n`;
    ...
    message += `\nCurrent Holdings\n`;
    message += ` ${formatTokenAmount(data.amt0, 6)} ${data.t0.symbol} ($${value0.toFixed(2)})\n`;
    message += ` ${formatTokenAmount(data.amt1, 2)} ${data.t1.symbol} ($${value1.toFixed(2)})\n`;
    message += ` Holdings: $${holdingsUSD.toFixed(2)}\n`;

    if (data.positionHistoryAnalysisSucceeded) {
        const holdingsChange = holdingsUSD - data.histPrincipalUSD;
        message += ` Holdings change: $${holdingsChange.toFixed(2)}\n`;
    }
    ...
    message += `\nUncollected Fees\n`;
    message += ` ${formatTokenAmount(data.fee0, 6)} ${data.t0.symbol} ($${feeUSD0.toFixed(2)})\n`;
    message += ` ${formatTokenAmount(data.fee1, 2)} ${data.t1.symbol} ($${feeUSD1.toFixed(2)})\n`;
    message += ` Total Fees: $${totalFeesUSD.toFixed(2)}\n`;
    ...
    if (data.positionHistoryAnalysisSucceeded && data.histPrincipalUSD > 0) {
        ...
        message += `\nFee Performance\n`;
        message += ` Fees per hour: $${(rewardsPerYear / 365.25 / 24).toFixed(4)}\n`;
        message += ` Fees per day: $${(rewardsPerYear / 365.25).toFixed(4)}\n`;
        message += ` Fees per month: $${(rewardsPerYear / 12).toFixed(4)}\n`;
        message += ` Fees per year: $${rewardsPerYear.toFixed(4)}\n`;
    ...
    if (data.positionHistoryAnalysisSucceeded) {
        ...
        message += ` Position Total return + Fees: $${totalReturn.toFixed(2)}\n`;
    }
